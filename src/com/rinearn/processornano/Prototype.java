...
/**
 * このコードは、本番の実装を作る前に挙動やUIなどを色々と検討するための、即席の試作版のコードです。
 *
 * そのため、単一のクラス内にほとんど全ての処理が雑にベタ書きされていますが、整理などは行いません。
 * 全体的なイメージが固まってきた段階で、このクラスの挙動やUIを元に、土台の部品から再実装を行います。
 * （その際の最上階層は {@link RinearnProcessorNano RinearnProcessorNano} クラスになります。）
 */
