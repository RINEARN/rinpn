...
1. Java&trade; Development Kit (19以降)
...
なお、RINPn の「 cmd 」フォルダのパスを環境変数 Path / PATH に登録しておくと、以下のように単純なコマンドで起動できます：
...
上の電卓画面上で、計算式を入力してPCの Enter キーを押すか、または電卓画面の「 = 」ボタンを押すと、その式の値を計算できます。また、PCの「Esc」キーか電卓画面の「C」ボタンを押すと、式をクリアできます。

式の中では、sin や cos など、各種の数学関数が標準で使用可能で、自作もできます。

また、式の代わりに、C言語風の簡易言語「 [Vnano](https://www.vcssl.org/ja-jp/vnano/doc/tutorial/language) 」で記述したスクリプトのファイル名/パスを入力する事で、それを実行する事もできます。「 Script 」ボタンを押して、画面上でスクリプトファイルを選ぶ事も可能です。スクリプトは、複雑な計算処理を自動化するのに便利です。

ところで、RINPn の cmd フォルダのパスを環境変数 Path / PATH に登録しておくと、以下のようにコマンドラインでも計算できます：
